...
:class:`~best_fit_plane.BestFitPlaneResult`.
...
one point it passes through (the centroid). To describe *where on the
...
class CoordinateSystemBuilder:
    ...
    @staticmethod
    def build_at_centroid(plane: BestFitPlaneResult) -> CoordinateSystem:
        """Build the local frame with its origin at the plane's centroid."""
        x_axis, y_axis, z_axis = _axes_from_rotation_matrix(plane.rotation_matrix)
        return CoordinateSystem(
            origin=plane.centroid.copy(),
            x_axis=x_axis,
            y_axis=y_axis,
            z_axis=z_axis,
            rotation_matrix=plane.rotation_matrix.copy(),
            reference=OriginReference.CENTROID,
            reference_point_index=None,
        )
