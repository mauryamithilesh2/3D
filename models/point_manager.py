...
class PointManager(QObject):
    ...
    def __init__(self, parent: QObject | None = None) -> None:
        ...
        self._last_edited_plane_label: str | None = None
        # ALL plane-point labels the user has manually edited (not just the
        # most recent one). The single-corner "auto-level" model in
        # core.tilt.virtual_leveled_points is only valid when exactly one
        # corner of a quad has actually been changed by the user -- with 2+
        # genuinely-edited corners it must be disabled so real edits are
        # never silently overwritten with a fabricated value.
        self._edited_plane_labels: set[str] = set()
    ...
    def remove_plane_point(self, label: str) -> None:
        ...
        self._edited_plane_labels.discard(label)
    ...
    def remove_last_plane_point(self) -> str | None:
        ...
        self._edited_plane_labels.discard(last_label)
    ...
    def update_plane_point(self, label: str, x: float, y: float, z: float) -> None:
        ...
        self._last_edited_plane_label = label
    ...
    def edited_plane_labels(self) -> set[str]:
        """All plane point labels the user has manually edited so far.

        Unlike ``last_edited_plane_label``, this is never overwritten -- it
        only grows -- so the auto-level model can tell the difference
        between "only one corner has ever been touched" and "two or more
        corners were genuinely, independently edited."
        """
        return set(self._edited_plane_labels)


    def set_reference_label(self, label: str | None) -> None:
        """Set which plane point acts as the fixed anchor for auto-relevel."""
        self._reference_label = label

    def update_plane_point_autolevel(self, label: str, x: float, y: float, z: float) -> None:
        """Set a corner's Z, then auto-level its two adjacent corners so all
        4 stay exactly coplanar: adjacent_z = (this_z + opposite_z) / 2.
        Works for any 4 point labels -- topology is resolved from geometry."""
        from core.tilt import resolve_quad_topology
        self.update_plane_point(label, x, y, z)
        topology = resolve_quad_topology(self._plane_points)
        info = topology.get(label)
        if not info:
            return
        z_opposite = float(self._plane_points[info["opposite"]][2])
        adjacent_z = (z + z_opposite) / 2.0
        for adj_label in info["adjacent"]:
            ax, ay, _ = self._plane_points[adj_label]
            self.update_plane_point(adj_label, ax, ay, adjacent_z)

    def apply_pivot_tilt(self, pivot_label: str, moving_label: str, lift_z: float) -> None:
        """Lift `moving_label` by lift_z, swinging it around `pivot_label` so the
        true 3-D distance between them is preserved exactly."""
        from core.tilt import pivot_tilt
        pivot = self._plane_points[pivot_label]
        moving = self._plane_points[moving_label]
        new_pos = pivot_tilt(pivot, moving, lift_z)
        self._plane_points[moving_label] = new_pos
        self._active_plane_points.add(moving_label)
        self.plane_points_changed.emit()
