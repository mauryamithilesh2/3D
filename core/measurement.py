...
from core.transform import CoordinateTransformer, LocalCoordinates
...
class MeasurementEngine:
    ...
    def measure_points(
        ...
        -------
        list[PointMeasurement]
            One measurement per input point, in the same order.
        """
        return [
...
def point_to_point_distance(point_a: np.ndarray, point_b: np.ndarray) -> float:
    ...
    Why does this live here rather than in ``utils/``?
        ``utils/`` is explicitly scoped to string parsing/formatting with
        no geometry of its own. A point-to-point distance is a measurement
        in the same sense as point-to-plane distance -- it belongs alongside
        the rest of this module's measurement math.
